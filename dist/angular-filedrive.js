...
angular.module("Filedrive", [ "ng-context-menu" ]);
...
angular.module("Filedrive").directive("filedriveListUi", [ "FiledriveController", function(controller) {
    return {
        ...
        template: '<div ng-if="state == \'loading\'" class="filedrive"><h2 class="ui center aligned header" ng-bind-html="options.loadingHtml"></h2></div><div ng-if="state == \'error\'" class="filedrive"><h2 class="ui center aligned header" ng-bind-html="options.errorHtml"></h2></div><div ng-if="state == \'show\'" class="filedrive">	<table class="ui striped table" context-menu data-target="context-menu-directory">		<thead>		<tr>			<th>Filename:</th>			<th>Oprettet:</th>			<th>Sidst redigeret:</th>			<th>Størrelse:</th>			<th>Type:</th>		</tr>		</thead>		<tbody>		<tr class="directory entity" ng-if="directory != options.directory">			<td><a href ng-click="openFile(getParentDirectory(directory), $event)">..</a></td>			<td></td>			<td></td>			<td></td>			<td></td>		</tr>		<tr ng-repeat="file in files" ng-class="{\'directory\': file.directory, \'file\': !file.directory, \'uploading\': file.uploading}" 			class="entity" ng-if="directory != file.path" context-menu data-target="context-menu-{{$index}}">			<td class="filename">				<a href ng-click="openFile(file, $event)" ng-if="!file.uploading">{{getFilename(file)}}</a>				<span ng-if="file.uploading"><i class="fa fa-spinner fa-pulse"></i> {{getFilename(file)}}</span>			</td>			<td class="created">				<a href ng-click="openFile(file, $event)" ng-if="!file.directory && !file.uploading">{{getDate(file.created_at, options.dateFormat)}}</a>				<span ng-if="file.uploading">{{getDate(file.created_at, options.dateFormat)}}</span>			</td>			<td class="last_modified">				<a href ng-click="openFile(file, $event)" ng-if="!file.directory && !file.uploading">{{getDate(file.last_modified, options.dateFormat)}}</a>				<span ng-if="file.uploading">{{getDate(file.last_modified, options.dateFormat)}}</span>			</td>			<td class="size">				<a href ng-click="openFile(file, $event)" ng-if="!file.directory && !file.uploading">{{getSize(file)}}</a>				<span ng-if="file.uploading">{{getSize(file)}}</span>			</td>			<td class="type">				<a href ng-click="openFile(file, $event)" ng-if="!file.directory && !file.uploading">{{getMimetype(file)}}</a>				<span ng-if="file.uploading">{{getMimetype(file)}}</span>			</td>		</tr>		</tbody>	</table>	<div ng-repeat="file in files" class="ui vertical menu context-menu" id="context-menu-{{$index}}" ng-if="directory != file.path">		<a class="item" href ng-click="renameFile(file)">{{options.renameContextMenuText}}</a>		<a class="item" href ng-click="newFolder()">{{options.newFolderContextMenuText}}</a>		<a class="item" href ng-click="deleteFile(file)">{{options.deleteContextMenuText}}</a>	</div>	<div class="ui vertical menu context-menu" id="context-menu-directory">		<a href class="item" ng-click="newFolder()">{{options.newFolderContextMenuText}}</a>	</div>	<filedrop class="filedrop" data-drop="dropFile" data-drop-html="options.dropHtml"></filedrop></div>',
        link: controller
    };
} ]);
...
angular.module("Filedrive").directive("filedrop", [ "$timeout", function($timeout) {
    var controller = function($scope, $element, $atts, ctrl) {
        $scope.drag = !1;
        var dragging = !1, timeoutHandler = null, onWindowDragOver = function(e) {
            onWindowDragEnter(e);
            ...
            dragging = !1;
            timeoutHandler && $timeout.cancel(timeoutHandler);
            timeoutHandler = $timeout(function() {
                dragging || ($scope.drag = !1);
            });
        };
        $scope.$watch("drag", function() {
            $scope.drag ? $element.show() : $element.hide();
        });
        ...
        $scope.dragElement = !1;
        var draggingElement = !1, timeoutElementHandler = null, onDragEnter = function(e) {
            draggingElement = !0;
            $scope.$apply(function() {
                $scope.dragElement = !0;
            });
            ...
            draggingElement = !1;
            timeoutElementHandler && $timeout.cancel(timeoutElementHandler);
            timeoutElementHandler = $timeout(function() {
                draggingElement || ($scope.dragElement = !1);
            });
        }, onDrop = function(e) {
            e.stopPropagation();
            e.preventDefault();
        ...
        $scope.$watch("dragElement", function() {
            $scope.dragElement ? $element.addClass("hover") : $element.removeClass("hover");
        });
...
angular.module("Filedrive").service("FiledriveService", [ "$q", "$filter", "$sce", function($q, $filter, $sce) {
    ...
    this.setupOptions = function($atts, options) {
        var attr_options = {};
        void 0 !== $atts.directory && (attr_options.directory = $atts.directory);
        for (var trustHtml = [ "loadingHtml", "errorHtml", "dropHtml", "deleteContextMenuText", "deleteConfirmText", "renameContextMenuText", "renamePromptText", "newFolderContextMenuText", "newFolderPromptText", "newFolderDefaultText" ], i = 0; trustHtml > i; i++) options[trustHtml[i]] && (options[trustHtml[i]] = $sce.trustAsHtml(options[trustHtml[i]]));
    ...
    this.getDate = function(date, format) {
        if (null === date || void 0 === date) return "";
        (null === format || void 0 === format) && (format = "dd-MM-yyyy");
    ...
    this.getFiletype = function(file) {
        ...
        var filename = self.getFilename(file), filetype = filename.substr(filename.lastIndexOf(".") + 1);
        return filetype;
    };
...
angular.module("Filedrive").service("FiledriveUploadCache", [ "FiledriveService", "$q", function(FiledriveService, $q) {
    ...
    this.addFile = function(file, cb) {
        ...
        var directory = FiledriveService.getParentDirectory(file.path).path, directoryFiles = files[directory] = files[directory] || [];
        file.promise.done(function() {
            var i = directoryFiles.indexOf(file);
            i > -1 && directoryFiles.splice(i, 1);
